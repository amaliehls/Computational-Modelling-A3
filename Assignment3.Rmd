...
#Simulate data from the posterior and plot it
plot( ADOS ~ VerbalIQ , data=data )
abline( a=coef(model_verbal)["a"] , b=coef(model_verbal)["b"] )

mu = link(model_verbal)

dens(mu)

ADOS.seq = seq(from = 0, to=25, by=1)

mu = link(model_verbal,data=data(ADOS = ADOS.seq))     
                 
                 sim.ADOS = sim(model_verbal,data=list(verbal))
